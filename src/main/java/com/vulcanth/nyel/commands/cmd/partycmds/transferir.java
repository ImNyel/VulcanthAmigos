...
public class transferir extends SubCommand {

    public transferir() {
        super("transferir", "transferir", "Transferir para algum membro da party", false);
    }

    public transferir(String name, String usage, String description, boolean onlyForPlayer) {
    ...
    public void perform(CommandSender sender, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (party.getPlayerRole(nick) == null) {
                sender.sendMessage(Arrays.toString(TextComponent.fromLegacyText(ChatColor.RED + "Esse jogador não está na party!")));
                return;

            }
            party.transferOwnership(nick);
            for (String playerNicks : party.getPlayers().keySet()) {
                if (ProxyServer.getInstance().getPlayer(playerNicks) == null) continue;
                ProxyServer.getInstance().getPlayer(playerNicks).sendMessage(TextComponent.fromLegacyText(ChatColor.YELLOW + "A liderança da party foi transferida para: " + Role.getPrefixed(nick)));
